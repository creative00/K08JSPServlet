...
public class LifeCycle extends HttpServlet {
	...
	/*
	서블릿 수명 주기에서 최조로 호출되는 메서드로
	어노테이션 통해 생성한다. 따라서 메서드명은 개발자가
	결정하면 된다. init()메서드가 호출되기 전 전처리 위해
	주로 사용된다. 
	 */
	@PostConstruct
	...
	전송 방식에 상관 없이 선 호출된 후 doGet()이나 doPost()를 호출한다. 
	 */
	@Override
	protected void service(HttpServletRequest req, HttpServletResponse resp)
		throws ServletException, IOException {
		System.out.println("service() 호출");
		/* service()메서드에서 요청 방식을 분석 후 각 메서드를 호출 시
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) 
		...
		req.getRequestDispatcher("/13Servlet/LifeCyle.jsp").forward(req, resp);
	}
