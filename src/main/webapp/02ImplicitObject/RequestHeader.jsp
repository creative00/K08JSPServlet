...
<body>
	...
	getHeaderNames() : 
		...
		hasMoreElements() :
		출력할 정보가 있는지 확인해 boolean값으로 반환한다.
	...
	*/
	...
	/*요청헤더를 통해 확인할 수 있는 값
	user-agent : 사용자가 접속한 웹 브라우저의 종류를 알 수 있다.
		크롬, 파이어폭스 등 여러 웹브라우저 통해 테스트해보면
		각기 다른 결과가 출력되는걸 볼 수 있다.
	referer : 웹서핑을 통해 특정한 링크를 클릭하여 다른 사이트로
		방문할 경우 남는 흔적을 말한다. 어떤 사이트 통해
		해당 페이지로 유입되었는지 알 수 있다.
	cookie : 웹서버가 브라우저 통해 클라이언트의 컴퓨터에 남기는
		흔적으로 파일 형태로 저장된다.
	*/
	%>
