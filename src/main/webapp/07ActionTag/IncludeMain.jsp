...
<body>
	...
	<!--
	지시어를 이용한 일클루드의 경우 페이지의 경로는
	문자열 형태로만 기술할 수 있다. 표현식을 통해
	변수를 사용할 수 없다.
	-->
	<h3>지시어로 페이지 포함하기</h3>
	...
	<!-- 지시어 통한 인클루드는 포함할 파일의 소스 그대로 
	현재 문서에 포함시킨 후 컴파일을 진행한다.
	따라서 하나의 페이지라 생각하면 된다. 그러므로 newVar1
	은 정상적으로 출력된다.  -->
	<p>외부 파일에 선언한 변수 :<%=newVar1%></p>
	
	<h3>액션 태그로 페이지 포함하기</h3>
	<!-- 액션태그를 이용한 인클루드는 문자열과 표현식
	두가지 모두 사용할 수 있다.
	-->
	<jsp:include page="./inc/OuterPage2.jsp" />
	<jsp:include page="<%=outerPath2%>" />
	
	<!--액션태그 통한 인클루드는 포함할 파일을 먼저 실행(컴파일)
	그 후 실행 결과를 해당 문서에 포함한다. 따라서 외부 파일에서
...
액션태그
	...
	공유된다.
	-HTML태그만 있거나 단독으로 실행 가능한 JSP코드만 있을 때 사용한다.  -->
</body>
</html>