...
<!--국제화(Formatting) 태그
: 국가별로 다양한 언어, 날짜, 시간, 숫자형식을 설정 시 사용한다.
  접두어로는 fmt를 사용한다.  -->
	...
	<!-- value속성값을 %단위로 변환해 출력한다. 즉 100을 곱한 결과에
	%기호까지 붙여준다.  -->
	...
	pattern: 반환할 문자열의 패턴을 지정해 파싱한다.
	integerOnly : 소수점 이하를 절삭해 정수부만 출력한다.
	 -->
	<h4>문자열을 숫자로 변경</h4>
	<c:set var="number2" value="6,789.01" />
	<!--
	pattern으로 주어진 부분을 분석해 문자열을 숫자형식으로 파싱한다.
	해당 속성값으로
	000,000.00 사용시 정상
	0,0.0 사용시 정상
	,. 사용시 에러 
