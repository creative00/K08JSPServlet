...
<head>
<meta charset="UTF-8">
<title>JSTL - if</title></head>
<body>
<!--
if태그 : 조건을 확인해 실행 여부를 판단한다.
속성정리
	test : EL을 이용해 조건식을 삽입
...
-->
	...
	<!--
	if(number%2==0) 과 공일한 조건의 if문으로 해당 조건의 결과가
	...
	EL에서는 eq를 통해 값에 대한 비교와 문자열에 대한 비교
	모두를 할 수 있다.여기서 사용한 eq는 Java의 compateTo()와
	같이 사전 순으로 문자열을 비교한다.  -->
	...
	result4 : ${ result4 } <br/>
	<!-- test에는 앞뒤에 하나라도 공백이 들어가면
	무조건 false를 반환하게 된다. 따라서 주의해야 한다.  -->
	...
	<h4>연습문제 : if태그 </h4>
	<!--
	아이디, 패스워드를 입력 후 submit버튼을 누르면
	EL식을 통해 파라미터를 받은 후 tjoeun/1234인 경우에는
	'tjoeun님, 하이룽~'이라고 출력한다.
	만약 틀렸다면 "아이디/비번을 확인하세요"라고 출력한다.
	EL과 JSTL의 if태그만을 이용해 구현하시오.
	-->
	...
	<c:if test="${ not empty param.user }">
		...
		<!--
		아이디, 패스워드 입력 시 일치 여부를 판단 후
		결과를 변수에 저장한다.
		if태그는 else구문이 따로 없으므로 not을 통해
		두번째 if문을 구성해야 한다.
		  -->
